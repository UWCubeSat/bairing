...
bool CalibratedBNO080::Initialize() {
  ...
  if (status) {
    _bno080.enableRotationVector(BNO080_PERIOD);  // quaternions; also enables calculation of eulers
    _bno080.enableMagnetometer(BNO080_PERIOD);    // magnetic field
    _bno080.enableAccelerometer(BNO080_PERIOD);   // acceleration including gravity
    _bno080.enableGyro(BNO080_PERIOD);            // angular rotation
  }
  return status;
}
